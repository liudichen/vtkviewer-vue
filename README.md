...
VtkViewer内置支持多种3D模型格式，通过相应的插件处理。同时提供了扩展接口，用户可以自行扩展对其他格式模型的支持。
...
安装后，插件会尝试自动加载Draco解码器（此方法不一定会成功，因为为了绕过vite的静态检测,对报名做了特殊处理,可能会忽略掉本地包,请优先采用方式一）。
事实上,如果都不配置,插件会尝试从路径`/draco_decoder.js`解析解码器，如果还不存在则会抛出异常。
...
```typescript
type I18nTranslateFunction = (key: string,options?:any) => string
```
...
你可以查看`BUILTIN_LANGS` 以了解完整的内置语言包。

### 6.6 在自定义插件中使用国际化

在自定义工具栏插件中，可以通过 `i18n` 单例进行翻译：
